...
app.get("/api/hospital-count", (req, res) => {
    ...
    app.get("/api/medical-history/:email", (req, res) => {
        ...
        const userQuery = `SELECT disease AS diseaseName, diseaseDetails AS diseaseDescription FROM users WHERE email = ?`;
...
>>>>>>> b855797 (Replaced previous files with updated versions)
            ...
            // Step 2: Fetch matching hospitals
<<<<<<< HEAD
            const hospitalQuery = `SELECT name, location, type FROM hospitals WHERE speciality LIKE ? LIMIT 2`;
            db.query(hospitalQuery, [`%${disease}%`], (err, hospitalResult) => {
                if (err) return res.status(500).json({ error: err.message });
    
=======
            const hospitalQuery = `SELECT * FROM hospitals WHERE speciality LIKE ?`;
            db.query(hospitalQuery, [`%${disease}%`], (err, hospitalResult) => {
                if (err) return res.status(500).json({ error: err.message });
        console.log(err)
>>>>>>> b855797 (Replaced previous files with updated versions)
                const response = [{
                    ...
                    severity: "Moderate", // Default or mocked severity
